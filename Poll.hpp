...
public:
	...
	void enroll(IStream *stream)
	{
		...
		cout << GRAY("Enroll ") << stream->getFD()
			 << (serv ? CYAN(" (ServerSocket)") : connected ? BLUE(" (ConnSocket)")
											  : P			? PURPLE(" (Pipe)")
															: "")
			 << endl;
	}
	...
	void drop(iterator it)
	{
		...
		TAG(PollSet, drop);
		cout << GRAY("Drop ") << it.first->fd << endl;
		delete (*it.second);
					 ...
					 we will drop iterator of Pipe and kill child process here.	*/
		{
			...
			TAG(PollSet, drop);
			cout << GRAY("Destroy linked pipe ") << (*itPipe)->getFD() << endl;
	...
	iterator examine()
	{
		...
		case -1:
			TAG(PollSet, examine);
			cerr << RED("poll() ERROR: ") << strerror(errno) << endl;
			break;
		case 0:
			TAG(PollSet, examine);
			cerr << GRAY("No event within ") << minRemaining << "ms" << endl;
			break;
		default:;
		}
		...
		for (iterator it = this->begin(); it < this->end(); it++)
		{
			...
			else
			{
				...
				if (it.first->revents & POLLIN)
					return readRoutine(it);
				if (it.first->revents & POLLOUT)
					return writeRoutine(it);
			}
		}
		throw exception();
	}
...
private:
	void print()
	{
		...
		for (iterator it = this->begin(); it < this->end(); it++, i++)
		{
			cout << "[" << i << "] ";
			cout << it.first->fd << " ( ";
			if (it.first->events & POLLIN)
				cout << "IN ";
			if (it.first->events & POLLOUT)
				cout << "OUT ";
			cout << ") ";
		}
		cout << endl;
		...
		if (serv)
		{
			while (1) // accept() will throw exception if not readied
			{
				try
				{
					...
					TAG(PollSet, examine);
					cout << GREEN("Got new connection, enroll ") << connected->getFD() << endl;
					connected->linkToServ = serv;
	...
	{
		TAG(PollSet, examine);
		cout << GREEN("Can write to ") << it.first->fd << endl;
		return it;
	}
};
